...
import { calculateItemEffects, MAX_ATTUNEMENT, canAttuneItem, requiresAttunement as itemRequiresAttunementFn } from './config/compatibilityRules.js';
import { calculateEquippedItemBonuses } from './config/fixedBonusItems.js';
...
export class EquipmentPopup {
    ...
    _findBestSlotForItem(item) {
        ...
        // Bastoni, bacchette e verghe magiche - equipaggiabili nelle mani
        if (category === 'staff' || category === 'wand' || category === 'rod') {
            // Se c'è già qualcosa in mainHand, prova offHand
            if (this.equippedSlots.mainHand && !this.equippedSlots.offHand) {
                return 'offHand';
            }
            return 'mainHand';
    ...
    _renderStatsSummary() {
        ...
        // Conteggio sintonizzazione
        const attunedCount = this.inventory.filter(i => i.isAttuned === true).length;
        
        // Calcola CA usando calculateItemEffects per gestire bonus magici
        const dexScore = this.character.abilities?.dexterity || this.character.abilities?.des || 10;
        const dexMod = Math.floor((dexScore - 10) / 2);
        
        let baseAC = 10;
        let armorAC = 0;
        let armorName = 'Nessuna';
        ...
        // Processa armatura nello slot body
        const bodyItem = this.equippedSlots.body;
        if (bodyItem) {
            const effects = calculateItemEffects(bodyItem, this.character);
            baseAC = effects.armorClass || 10;
            armorAC = baseAC - 10;
            armorName = bodyItem.name || 'Armatura';
            
            if (effects.armorClassDexBonus) {
                if (effects.armorClassDexMax !== null) {
                    dexBonusForAC = Math.min(dexMod, effects.armorClassDexMax);
                } else {
            ...
            // Bonus magico dell'armatura
            if (bodyItem.magicBonus?.armorClass) {
                magicBonus += bodyItem.magicBonus.armorClass;
                magicItemNames.push(bodyItem.name);
            }
        }
        
        // Processa scudi (mainHand o offHand)
        ['mainHand', 'offHand'].forEach(slotId => {
            const slotItem = this.equippedSlots[slotId];
            if (!slotItem) return;
            
            const isShield = slotItem.equipment_category?.index === 'shield' || 
                             slotItem.name?.toLowerCase().includes('scudo') ||
                             slotItem.name?.toLowerCase().includes('shield') ||
                             slotItem.armor_category?.toLowerCase().includes('scudo');
            
            if (isShield) {
                const shieldBase = slotItem.armor_class?.base || 2;
                const shieldMagic = slotItem.magicBonus?.armorClass || 0;
                shieldBonus += shieldBase + shieldMagic;
                shieldName = slotItem.name || 'Scudo';
                
                if (shieldMagic > 0) {
                    magicBonus += shieldMagic;
                    magicItemNames.push(slotItem.name);
                }
            }
        });
        
        const totalAC = 10 + armorAC + dexBonusForAC + shieldBonus;
        
        // Build breakdown string
        let breakdownParts = [];
        if (armorAC > 0) breakdownParts.push(`${armorName} ${baseAC}`);
        if (dexBonusForAC !== 0) breakdownParts.push(`DES ${dexBonusForAC >= 0 ? '+' : ''}${dexBonusForAC}`);
        if (shieldBonus > 0) breakdownParts.push(`${shieldName} +${shieldBonus}`);
        const breakdownStr = breakdownParts.length > 0 ? breakdownParts.join(' + ') : 'base 10';
        ...
        return `
            <div class="stats-summary">
                ...
                <div class="stat-row ${attunedCount > 0 ? 'stat-row-attuned' : ''}">
                    <span class="stat-label">Sintonizzati:</span>
                    <span class="stat-value">${attunedCount}/${MAX_ATTUNEMENT}</span>
                </div>
                <div class="stat-row stat-row-ac">
                    <span class="stat-label">CA Totale:</span>
                    <span class="stat-value stat-value-ac">${totalAC}</span>
                    <span class="stat-breakdown">(${breakdownStr})</span>
                </div>
                ${magicBonus > 0 ? `
                    <div class="stat-row stat-row-magic">
                        <span class="stat-label">Bonus Magico:</span>
                        <span class="stat-value stat-value-magic">+${magicBonus}</span>
                        <span class="stat-breakdown">${magicItemNames.join(', ')}</span>
                    </div>
                ` : ''}
                <div class="stat-row">
    ...
    _addFromDatabase(itemIndex) {
        ...
        if (existingIdx >= 0) {
            ...
            this.inventory.push({
                ...
                equipped: false,
                equippedSlot: null,
                isAttuned: false  // Sintonizzazione non attiva di default
            });
    ...
    /**
     * Togglie la sintonizzazione di un oggetto magico.
     * Rispetta il limite di MAX_ATTUNEMENT (3 oggetti).
     */
    _toggleAttunement(itemIndex) {
        ...
        // Verifica che l'oggetto richieda sintonizzazione
        if (!itemRequiresAttunementFn(item)) {
            showToast('Questo oggetto non richiede sintonizzazione.', 'warning');
            return;
        }

        if (item.isAttuned) {
            // Rimuovi sintonizzazione
            item.isAttuned = false;
            showToast(`⚡ ${item.name} — sintonizzazione rimossa`, 'info');
        } else {
            // Aggiungi sintonizzazione — controlla limite
            const currentAttuned = this.inventory.filter(i => i.isAttuned === true).length;
            if (currentAttuned >= MAX_ATTUNEMENT) {
                showToast(`⚠️ Limite sintonizzazione raggiunto (${MAX_ATTUNEMENT}/${MAX_ATTUNEMENT}). Rimuovi un oggetto sintonizzato prima.`, 'error');
                return;
            }
            item.isAttuned = true;
            showToast(`🔗 ${item.name} — sintonizzato! (${currentAttuned + 1}/${MAX_ATTUNEMENT})`, 'success');
        }

        // Sincronizza la modifica anche nella copia in equippedSlots (evita snapshot obsoleto)
        if (item.equipped && item.equippedSlot && this.equippedSlots[item.equippedSlot]) {
            this.equippedSlots[item.equippedSlot].isAttuned = item.isAttuned;
        }

        // Ricalcola stats (i bonus dipendono dalla sintonizzazione)
    ...
    _refreshDetailPanel() {
        ...
        if (detailContainer && this.selectedItem) {
            // Passa l'indice dell'oggetto selezionato per il toggle sintonizzazione
            this.selectedItem._inventoryIndex = this.selectedItemIndex;
    ...
    _save() {
        ...
        // Ricostruisci equippedSlots da inventory prima di salvare
        // (evita snapshot obsoleti se sono state fatte modifiche post-equip)
        this._buildEquippedSlotsFromInventory();
        ...
        // Calcola statistiche equipaggiamento e aggiorna i valori del personaggio
        this._updateCharacterStats();
    ...
    /**
     * Aggiorna le statistiche del personaggio in base all'equipaggiamento.
     * Propaga i bonus magici alle proprieta del personaggio (CA, ecc.)
     * in modo che vengano riflessi nelle schede PG e PNG.
     */
    _updateCharacterStats() {
        // Calcola bonus da oggetti magici equipaggiati
        let itemBonuses = null;
        let effectiveAbilities = this.character.abilities || {};
        try {
            itemBonuses = calculateEquippedItemBonuses(this.character);
            effectiveAbilities = itemBonuses.effectiveAbilities;
        } catch { /* modulo non disponibile */ }

        const dexScore = effectiveAbilities?.dexterity || 10;
        const dexMod = Math.floor((dexScore - 10) / 2);
        ...
        ['mainHand', 'offHand'].forEach(slotId => {
            ...
            if (isShield) {
                ...
                const shieldMagic = slotItem.magicBonus?.armorClass || 0;
                shieldBonus += shieldBase + shieldMagic;
                if (shieldMagic > 0) magicBonus += shieldMagic;
            }
        });
        
        // Aggiungi bonus CA piatto da oggetti magici (Bracciali Difesa, Mantello Protezione, ecc.)
        const itemACBonus = itemBonuses?.flatACBonus || 0;
        const totalAC = baseAC + dexBonusForAC + shieldBonus + itemACBonus;
        if (this.character.armorClass !== undefined) {
            this.character.armorClass = totalAC;
        }
        if (this.character.ac !== undefined) {
            this.character.ac = totalAC;
        }
    }
}
