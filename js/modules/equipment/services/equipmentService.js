    ...
    calculateStats() {
        ...
        Object.entries(this.equipped).forEach(([slotId, item]) => {
            ...
            if (slotId === 'body' && effects.armorClass) {
                ...
                // Bonus magico armatura (oggetti con magicBonus strutturato o rarity)
                if (effects.armorClassBonus && (item.magicBonus?.armorClass || item.rarity)) {
                    stats.armorClass.magicBonus += effects.armorClassBonus;
                }
            }
            
            // Scudi (slot mainHand o offHand)
            if ((slotId === 'mainHand' || slotId === 'offHand')) {
                ...
                if (isShield) {
                    // Scudo base +2, + il bonus magico se presente
                    const shieldBase = 2;
                    const shieldMagic = (item.magicBonus?.armorClass) || 0;
                    stats.armorClass.shield += shieldBase + shieldMagic;
                }
            }
