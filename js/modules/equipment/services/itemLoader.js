...
export function getCompatibleSlotsForItem(item) {
    ...
    // Bastoni magici (es. Bastone del Potere) - equipaggiabili come armi
    // Un bastone da combattimento ha proprietà versatile (1 o 2 mani)
    if (category === 'staff') {
        slots.push('mainHand', 'offHand');
    }
    
    // Bacchette e Verghe magiche - equipaggiabili a una mano
    if (category === 'wand' || category === 'rod') {
        slots.push('mainHand', 'offHand');
    }
 ...
 */
const EQUIPPABLE_CATEGORIES = ['weapon', 'armor', 'staff', 'wand', 'rod'];
...
const WONDROUS_NON_WEARABLE_KEYWORDS = [
    ...
    // Note: bacchetta/verga/wand/rod/staff/bastone sono gestiti come
    // categorie equipaggiabili (vedi EQUIPPABLE_CATEGORIES) e non qui.
 ...
 * 3. È un bastone/bacchetta/verga magica (staff, wand, rod)
 * 4. È un oggetto meraviglioso INDOS ABILE (anelli, amuleti, cinture, ecc.)
 * 
...
export function isItemEquippable(item) {
    ...
    // 1. Armi, armature, bastoni, bacchette e verghe sono sempre equipaggiabili
