 ...
 * 
 * @author DM Tool
 * @version 1.1.0 - Supporto bonus oggetti magici fissi
 */
 ...
 * Include bonus CA piatto da oggetti magici (es. Bracciali della Difesa +2)
 * @param {Object} pg - Dati del personaggio
 * @param {Array} itemsDatabase - Database degli oggetti (opzionale, per lookup armature)
 * @returns {Object} - { ac, armorName, shieldBonus, hasShield, hasArmor, itemACBonus, itemSaveBonus, itemCheckBonus }
 */
export function calculateArmorClass(pg, itemsDatabase = null) {
    // Calcola le abilità efficaci (incluse quelle modificate da oggetti magici)
    let effectiveDex = pg.abilities?.dexterity || 10;
    let _itemACBonus = 0;
    let _itemSaveBonus = 0;
    let _itemCheckBonus = 0;
    try {
        const itemBonuses = calculateEquippedItemBonuses(pg);
        if (itemBonuses) {
            effectiveDex = itemBonuses.effectiveAbilities?.dexterity || effectiveDex;
            _itemACBonus = itemBonuses.flatACBonus || 0;
            _itemSaveBonus = itemBonuses.totalSaveBonus || 0;
            _itemCheckBonus = itemBonuses.totalCheckBonus || 0;
        }
    ...
    // Aggiungi bonus CA piatto da oggetti magici (es. Bracciali Difesa +2)
    ac += _itemACBonus;
    ...
    return {
        ...
        hasShield: !!shield,
        hasArmor: !!armor,
        itemACBonus: _itemACBonus,
        itemSaveBonus: _itemSaveBonus,
        itemCheckBonus: _itemCheckBonus
    };
}
